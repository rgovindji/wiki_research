...
## Recent developments
- **2026-07-01 — Patel corroborates *and reframes* the multi-partner TPU picture: three *distinct architectures*, not just three SerDes vendors** (per [[2026-07-01-sequoia-dylan-patel-hw-sw-codesign]]; *bias: Sequoia hosts long the AI complex*). Dylan Patel says Google runs **three different TPU design programs — Broadcom, MediaTek, and an undisclosed third — and they are *different architectures*, not the same TPU built by different partners.** He frames the whole industry as headed toward everyone deploying tens of $B (Google *hundreds* of $B/yr) of custom silicon while keeping general-purpose NVDA as a hedge against model-architecture "local minima." **Which means:** the ASIC TAM Broadcom addresses is bigger and more architecturally fragmented than a single-TPU-line read implies — *supportive* of the custom-silicon secular thesis — but it also reinforces the 6/30 caution below that Broadcom's socket is contestable and Google is deliberately multi-sourcing (MediaTek/Marvell). Net: bullish the ASIC-TAM backdrop, neutral-to-mild-caution on Broadcom's share of Google specifically. Stance held **bull/high**.
- **2026-06-30 — The TPU diversification widens: Google reportedly in talks to add Marvell as a *third* design partner** ([[2026-06-30-google-tpu-third-partner-marvell]]). Following the v9 SerDes loss to MediaTek (below), [[MRVL]] is now rumored (unsigned) for a Google **inference TPU + memory-processing unit** — a multi-supplier "no single vendor gets leverage" architecture. **Which means:** incremental confirmation that Broadcom's TPU socket is *contestable* and Google is deliberately commoditizing its design-partner layer — but **Broadcom remains the anchor** (training chip "8t", the newly-locked **through-2031** TPU agreement, Anthropic's Ironwood v7). Net: a mild share-of-wallet caution on the TPU line, not a thesis-breaker; the OpenAI/Meta/ByteDance XPU legs are unaffected. Stance held **bull/high**.
- **🔥 2026-06-24 — OpenAI × Broadcom unveil "Jalapeño" (OpenAI's first custom inference chip) — but a TPU v9 SerDes loss and a $29B lease guarantee land the same day** ([[2026-06-24-semiwiki-daily]]; CNBC/OpenAI/Broadcom joint release). **(1) The win:** Jalapeño is OpenAI's first Intelligence Processor and the first in a **multi-generation** compute platform the two are building together — **9 months design→tape-out** (among the fastest high-perf ASIC cycles ever, sped by OpenAI's own models), perf/watt "substantially better than state-of-the-art," initial deploy **end-2026**, **MSFT ~40%** of volume. AVGO +~3% intraday, **closed +0.51% ($382.07)**. This is the concrete realization of the Project Titan / OpenAI XPU leg the bull case has carried. **(2) The quieter loss:** Broadcom reportedly **lost the Google TPU v9 SerDes socket to MediaTek** — Google fell back from a 448G target to MediaTek's **336G** on signal-integrity/thermal walls; AVGO's 448G DSP slips to a 2nm / **2028-29** problem. **(3) The new liability:** Broadcom's 10-Q (Note 11) discloses a **$29B contingent guarantee** backstopping **Apollo's** purchase of OpenAI racks (Broadcom = guarantor if OpenAI defaults) — a balance-sheet subordination to OpenAI revenue, an [[ai-infrastructure-debt]] vector. **Which means**: the custom-silicon moat is both validated (OpenAI multi-gen) and shown contestable (Google socket to MediaTek) in one session, while AVGO takes on deployment-underwriter risk it didn't carry before. Net thesis-positive on the OpenAI win but no longer unalloyed. Stance held **bull / high**.
...
- **🔥 2026-05-17 PM — Druckenmiller INITIATED AVGO position Q1 2026** (Duquesne 13F via [[2026-05-17-robotics-multi-agent-batch]] context). **Druckenmiller simultaneously EXITED GOOGL** (385K shares) and slashed Amazon 97% (737K → 9.5K) — major rotation **out of Alphabet/Amazon hyperscaler giants INTO Broadcom**. Combined with Druckenmiller's known long-copper-futures + March 2026 Morgan Stanley interview ([[2026-05-16-druckenmiller-hard-lessons]]), this signals **Druckenmiller is pivoting from hyperscaler giants toward infrastructure/chipmaking enablers**. Bull thesis sharpens — when Druckenmiller initiates, the position deserves attention.
