---
type: company
ticker: CRWV
tags: [ai, neocloud, gpu-cloud, nvda-direct]
last_updated: 2026-07-01
last_full_review: 2026-05-13
sources: 8
conviction: high
stance: bull
---

# CoreWeave (CRWV)

**Stance:** bull · **Conviction:** **high** (upgraded from medium 2026-05-13 after Q1 2026 blow-out) · **Time horizon:** long-term + medium-term tactical

## One-line thesis
The largest "neocloud" — leases out high-performance GPU compute to AI labs, hyperscalers, and enterprises. **NVDA anchored the IPO with $250M + recently placed $2B more in Class A** for a 5+ GW AI factory partnership by 2030. **Q1 2026 print decisively re-rates the thesis: revenue $2.078B (+112% YoY), backlog $99.4B (~50% QoQ growth, ~2x [[NBIS]]'s $44.4B), Adj EBITDA $1.157B (56% margin), Anthropic multi-year Claude contract NEW disclosure, Meta added $21B in March.**

> **What this means:** CoreWeave has the largest contracted backlog ($99.4 billion) of any AI infrastructure pure-play in public markets. They run Anthropic's Claude (frontier-grade workload), they have 1+ gigawatt of active power, and they're building toward 8+ gigawatts by 2030. The previous "speculative AI infra" framing is wrong — this is a hyperscaler-quality scaled infrastructure business now. The remaining bear case is execution at the capex pace ($7.7B in one quarter) and debt service ($24.86B total debt).

## Role in the NVDA stack
- **Neocloud / GPU cloud** — Tier-2 buyer of NVDA hardware, reselling GPU-hours to AI customers
- **NVDA $250M IPO anchor order + $2B Class A placement** — deepest NVDA tie outside [[NBIS]]
- **NVIDIA Exemplar Cloud designation for GB200 NVL72 inference** (Q1 2026)
- **NVDA partnership for 5+ GW AI factory buildout by 2030**
- Customer base now disclosed includes **Microsoft, Meta, OpenAI, Anthropic, Cohere, Mistral, Jane Street, Perplexity, World Labs, Hudson River Trading**
- **2026 revenue guide $12-13B** (~4x [[NBIS]]); 2026 exit ARR $18-19B; >$30B ARR target EOY 2027

## Bull case
- **Q1 2026 blow-out validates the thesis (NEW — see [[2026-05-07-CRWV-Q1-2026-earnings]]).** Revenue **$2.078B (+112% YoY)**; Adjusted EBITDA **$1.157B (56% margin, +91% YoY)**; **backlog $99.4B (~50% QoQ growth)** — largest contracted backlog of any AI infrastructure pure-play in public markets. Operating cash flow $2.984B. Q1 2026 guide $12-13B full-year; exit 2026 at $18-19B ARR; >$30B ARR by EOY 2027. **Which means:** CRWV has the largest contracted backlog of any AI infra pure-play in public markets and is profitable at scale on the operating measure that matters (Adj EBITDA).
- **Anthropic multi-year Claude contract (NEW Q1 2026 disclosure).** Anthropic at $30B+ ARR running frontier-grade Claude workloads on CRWV is the highest-leverage validation of CRWV's enterprise positioning. **Which means:** Anthropic's choice signals goodput / reliability competitive with hyperscalers — see [[2026-04-20-semianalysis-gpu-cluster-cost]] ClusterMAX framework.
- **Meta added new $21B commitment in March 2026** (incremental to prior $35B-class contracts). **NVDA $2B Class A placement + 5+ GW partnership by 2030.** **Which means:** the top three AI buyers globally (NVDA, Meta, Anthropic) all anchor CRWV — that's customer mix few infrastructure providers can match.
- **Pure-play GPU compute** supplier with hyperscaler-scale contracts
- **First-mover in neocloud** with NVDA-blessed positioning
- **Capital-intensive moat** — building data centers fast at scale is hard to replicate
- **NVDA partnership** = priority on GPU allocations
- **GPU rental rates +40% (NEW — see [[2026-04-01-semianalysis-gpu-rental-index]]).** H100 1-yr contracts rose from $1.70 → $2.35/hr (Oct 2025 → Mar 2026), still rising 15-20% MoM. **All capacity through Aug-Sep 2026 already booked, market-wide.** Some H100 contracts being renewed at the exact same rate they were signed at 2-3 years ago. **Which means:** CRWV's unit economics are improving rapidly through 2026 contract repricing — shorter-duration contract books reprice into the higher-rate environment fastest. The "GPU rental softening / commoditization" bear case is being refuted in real-time by the authoritative SemiAnalysis survey index.
- **Sentiment vs fundamentals disconnect FINALLY RESOLVED.** April SemiAnalysis "Long NBIS" comment and parallel "sentiment vs fundamentals" framing applied to CRWV too — Q1 print is the catalyst that re-rates the entire Neocloud cohort.
- **Cluster goodput differentiation (NEW — see [[2026-04-20-semianalysis-gpu-cluster-cost]]).** SemiAnalysis ClusterMAX 2.1 framework rates Neoclouds on goodput (effective compute output), not just raw $/GPU-hr. **Which means:** the "all clouds will commoditize" bear case is structurally wrong — providers with measurable uptime/performance advantages command premiums and longer-tenor contracts.
- **Latent demand vector — off-board robot inference (NEW 2026-05-16, light bullet).** Per Sergey Levine (Physical Intelligence) [[2026-05-16-sergey-levine-physical-intelligence]]: low-cost robots will likely stream sensor data to **cloud GPUs**, with on-board inference reserved for outdoor / reliability-critical use cases. A billion robots in 2030+ each pulling control commands at sub-second cadence = a new compute load class (high QPS, low-batch, latency-sensitive), additive to LLM training/inference. **Which means:** beyond the current LLM-training-and-inference framing, CRWV has a *second* demand vector emerging on the 2028-2030 horizon. Not yet priced anywhere. Watch for any CRWV / hyperscaler disclosure of robotics-cloud-inference SKUs as the leading indicator.

> **What this means (bull case synthesis):** CoreWeave just printed $2 billion in quarterly revenue with $100 billion in backlog, signed Anthropic to a multi-year Claude contract, and has more than 1 GW of active AI compute running. They are no longer a "neocloud bet" — they are a hyperscaler-quality scaled infrastructure business with the largest AI-specific backlog in public markets. The remaining risk is purely execution at the capex pace, not "does the business model work."

## Bear case / risks
- **Capex burn rate is extreme.** **Q1 2026 capex $7.695B** in a single quarter — ~$31B annualized pace. **Cash went from $3.127B → $2.244B**; **total debt rose $21.4B → $24.9B (+$3.5B)**. Financed via DDTL 4.0 $8.5B facility + $2B NVDA Class A placement + operating cash flow. **Which means:** CRWV can fund this pace for ~12-18 months but **rising rates (see [[fed-policy]] and [[2026-05-13-ppi-cpi-shock-positioning]]) materially increase refinancing risk**. The 5.9% fixed rate on DDTL is fine; if 10y goes to 5%+ and the company needs to refi at higher spreads, FCF compresses.
- **Operating margin compressed despite EBITDA scaling.** Adj EBITDA margin 62% → 56% (Y/Y); Adj operating income only $21M (vs $163M Y/Y) due to capex-driven D&A. Margin trough per management guidance — needs Q2 sequential expansion to validate.
- **Customer concentration extreme.** MSFT + Meta + OpenAI + Anthropic = effectively the entire backlog. Loss of any one re-rates valuation.
- **Capital-intensive model** — high debt loads, capex cycles, fragile if rates rise / GPU prices fall
- **GPU resale risk** — older GPUs depreciate fast as Blackwell → Rubin → next ramps
- **Margin pressure** — competing against hyperscalers and other neoclouds (NBIS, Lambda, etc.) on price
- **Hyperscaler insourcing** — MSFT/META could pull workloads back in-house

> **What this means (bear case synthesis):** CRWV's bear case is no longer "does the business work?" — Q1 proved it does. The bear case is **financial leverage in a rising-rate regime**. They're funding ~$31B of annual capex via cash + debt + equity, and if rates stay higher-for-longer (per the morning's [[2026-05-13-ppi-cpi-shock-positioning]] analysis), refinancing risk compounds. Customer concentration is the second-order risk; rate sensitivity is first-order.

## Key questions / what to watch
1. **Q2 2026 print** — does Adj operating income expand from $21M sequentially per guide ($30-90M Q2)?
2. **Capex pacing** — does Q2 capex sustain ~$7.5B+ or moderate? Critical for cash burn math
3. **Customer concentration disclosure** — any single-customer % of revenue?
4. **Anthropic spend ramp** — what % of Anthropic's $30B+ ARR runs on CRWV?
5. **Power milestones** — 8+ GW by 2030 target; Q-over-Q progress
6. **Debt service trajectory** — refinancing risk if 10y Treasury sustains above 4.5% (see [[fed-policy]])

## Recent developments
- **2026-07-01 — Patel explains *why the neocloud opportunity exists at all* — and says CoreWeave's compute tests *better* than the hyperscalers'** (per [[2026-07-01-sequoia-dylan-patel-hw-sw-codesign]]; *bias: Sequoia hosts long the neocloud complex incl. Crusoe*). The structural case: AI GPU rental mechanics (whole-rack, long-duration contracts; no time-splicing) **stripped away the hyperscalers' CPU-cloud moats** — Amazon's Nitro NICs *hurt* AI performance, security/tenant-isolation is irrelevant when nobody rents a fraction of a rack, and Microsoft's own DC teams "fell on their face" when they had to double the forecast → had to buy *neocloud* capacity. Patel: **CoreWeave's GPU compute tested objectively better on performance/reliability** than Amazon/Google/Microsoft; the neocloud edge is *time-to-market* (hyperlevered equity owners who deliver faster). Hard economics behind the model: GPUs rent at **~$12–13B/GW** vs Trainium <$10B; colo power-only went **$60 → $120–160/kW-mo** ($200 bad-credit). **Which means:** an independent, mechanism-level rebuttal to the Chanos "dumb-shell, single-digit-ROIC" short below — the opportunity is real *because* AI economics inverted the hyperscalers' historical advantages, and CoreWeave is named as a genuine top-tier operator (not a commodity shell). Weigh against host bias (Sequoia is long the complex) and the standing leverage/financing-treadmill risk ([[ai-infrastructure-debt]]). Stance held **bull/high**, small-size discipline unchanged.
- **2026-06-28 — Fresh SemiAnalysis corroboration of the goodput moat** ([[2026-06-28-semianalysis-true-cost-gpu-cluster]]; *bias MODERATE — funnels to ClusterMAX*). Re-states that two clusters with identical GPUs/price can differ **10–50% in real TCO**, with **goodput** (useful work after failures/restarts) the dominant hidden cost — reliability *worsens* with scale, so detect/replace-failed-node speed and fault-tolerance (TorchFT / HyperPod checkpointless / TorchPass) decide TCO, not $/GPU-hr. **Which means:** independent restatement of the structural rebuttal to the Chanos "dumb-shell, single-digit-ROIC" short below — premium goodput-tier operators can command pricing/tenor the cheapest sticker can't. Stance held **bull/high**, small-size discipline unchanged.
- **2026-06-23 — Chanos's central short thesis names this category: "equipment-leasing/finance cos at single-digit ROIC"** ([[2026-06-23-chanos-zlatev-ai-capex-debate]]). Jim Chanos — *who is short neoclouds/miner-DCs but NOT short any semiconductor* — models the neocloud category as **equipment-leasing finance companies, not tech** ("the CoreWeave guys are the old Magnetar guys; Blackstone is now in the business"), penciling to **4–8% pre-tax ROIC even on a generous 10-year GPU life**. His tell: hyperscalers *can* buy chips directly but choose to rent → gold-rush capacity grab that compresses once supply catches up. **Counter (Val Zlatev, same panel):** not all dumb shells — CoreWeave has *real* software/optimization layers above the NVDA stack; the near-term spot-rent inflection (+40–50% since Jan, see [[inference-economics]]) is *improving* neocloud economics right now. **Which means:** the bear is the *structural ROIC* (value migrates to chip tech, not landlords) while the bull is *cyclical spot tightness* — exactly the franchise-vs-rent fork this page already carries via the financing treadmill ([[ai-infrastructure-debt]]). Stance held **bull/high**, small-size discipline reaffirmed; both speakers are interested parties (Chanos short, Zlatev net-long).
- **2026-06-16 — +9.7% to $117.03 on three real catalysts (not "compute headlines")** ([[2026-06-16-baker-bottleneck-rotation-boj-hike]]). (1) **Nasdaq-100 inclusion effective June 22** → mechanical passive/benchmarked buying; (2) a **$2.2B, 15-year Chicago hyperscale data-center lease** (contracted revenue + visibility); (3) **Cantor Fitzgerald $167 PT**, calling it cheap at ~$115; plus 90% of the 2027 ARR target now secured. Intraday ran +12-16% then faded as CRWV sounded out EUR/USD high-yield bond demand (the financing-treadmill reminder). **Which means:** on a day the AI *supply-side* book was routed (semis −4% to −10%), CRWV rose on *demand-side*, company-specific, contracted catalysts — a clean illustration of the Baker supply→demand rotation (see [[bottleneck-roadmap]]). Note the standing tension from the Q2 guide miss + the recurring debt-market activity ([[ai-infrastructure-debt]]); stance held **bull/high** but the leverage is why position sizing stays small.
- **2026-05-07** — **Q1 2026 EARNINGS BLOW-OUT.** Revenue **$2.078B (+112% YoY)**; **backlog $99.4B (+~50% QoQ)**; Adj EBITDA **$1.157B (56% margin)**; **Anthropic multi-year Claude contract** NEW disclosure; **Meta added $21B in March 2026** (incremental); operating cash flow $2.984B; capex $7.695B. NVDA $2B Class A placement + 5+ GW partnership by 2030. Active power >1 GW; contracted >3.5 GW; 2030 target 8+ GW. 2026 guide $12-13B; exit ARR $18-19B; >$30B ARR EOY 2027. NVIDIA Exemplar Cloud designation for GB200 NVL72 inference. See [[2026-05-07-CRWV-Q1-2026-earnings]].
- **2026-05-13** — Wiki retroactively processed Q1 print after user flagged it had been missed. Triggered new always-on earnings retrieval workflow.
- **2026-05-17 PM — Q2 2026 guidance TRAILED consensus** ([[2026-05-17-multi-agent-research-batch]] Agent F): **$2.53B guide vs $2.69B consensus** (-6% guide miss). Stock still +80% YTD despite miss. Wells Fargo PT raised to $155 (from $135); Deutsche Bank to $135 (from $125); 33 analysts Buy consensus. **NVDA bought another $2B in CRWV shares post-Q1**. 10 customers committed >$1B; 36% of $99.4B backlog to recognize in next 24 months. **Bull tension**: extraordinary backlog growth + NVDA capital commitment + customer diversification (10 $1B+ customers) vs guidance miss + premium valuation. **Agent recommended conviction NEUTRAL** (down from high). **House view**: keeping bull / high pending Q2 actual print and any updated 2026 ARR exit trajectory — but the Q2 miss is a yellow flag that warrants closer monitoring on capex execution and customer ramp pace.

## Related
[[NVDA]] · [[nvda-supply-chain]] · [[NBIS]] · [[ORCL]] · [[AMD]] · [[META]] · [[ai-capex-cycle]] · [[fed-policy]] · [[2026-05-13-ppi-cpi-shock-positioning]] · [[overview]]

## Sources
1. [[2026-05-07-CRWV-Q1-2026-earnings]] — official Q1 2026 press release; backlog $99.4B; Anthropic Claude deal disclosure
2. [[2026-04-01-semianalysis-gpu-rental-index]] — H100 1-yr rental price index, +40% in 5 months, all capacity booked through Sep 2026, sentiment-vs-fundamentals disconnect
3. [[2026-05-17-multi-agent-research-batch]] — Q2 2026 guidance miss ($2.53B vs $2.69B consensus); NVDA $2B additional share buy; 10 $1B+ customers; agent recommended neutral conviction (added 2026-05-17 PM)
3. [[2026-04-20-semianalysis-gpu-cluster-cost]] — ClusterMAX 2.1 framework; goodput-based differentiation refutes commoditization bear case
4. [[2026-05-16-sergey-levine-physical-intelligence]] — off-board robot inference as latent 2028-2030 demand vector
5. [[2026-06-23-chanos-zlatev-ai-capex-debate]] — Chanos's neocloud short thesis (equipment-leasing finance cos, 4-8% ROIC on 10yr GPU life; "the Magnetar guys") + Zlatev's software-layer counter (added 2026-06-23; bias-flagged: Chanos short, Zlatev net-long)

## Citations
- 24/7 Wall St on NBIS as neocloud winner: https://247wallst.com/investing/2026/03/11/after-2-billion-nvidia-investment-nebius-group-just-became-the-real-neocloud-winner/
- Motley Fool Meta $48B with CRWV+NBIS: https://www.fool.com/investing/2026/04/21/meta-is-spending-48-billion-with-coreweave-nebius/
- TheStreet BofA on CRWV+NBIS: https://www.thestreet.com/investing/stocks/bank-of-america-revamps-nvidia-backed-coreweave-and-nebius-stocks
- Gurufocus NBIS vs CRWV: https://www.gurufocus.com/news/8791476/nebius-vs-coreweave-which-is-the-better-ai-stock-for-2026
- Intellectia neocloud analysis: https://intellectia.ai/blog/neocloud-stocks-investment-analysis-2026
- 24/7 Wall St on AI infra rally: https://247wallst.com/investing/2026/03/31/coreweave-surges-8-nebius-gains-7-ai-infrastructure-stocks-rally-on-landmark-financing-and-platform-news/
