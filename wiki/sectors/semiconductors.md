...
## Recent developments
- **2026-06-30 — SemiWiki/feeds sweep: photonics' scaling gate is *repeatable* test, China memory tenders equipment in size, an HBM-relief research tell** ([[2026-06-30-semiwiki-feeds-daily]]). Four diamonds. **(1) Photonics (standing interest):** Kolbehdari's framing — silicon photonics has solved lab-grade alignment, but the CPO/optical-I/O/PIC realization boundary is **repeatability** (align + measure + calibrate light thousands of times across wafers/operators/lots); photonics probe stations are the R&D→manufacturing-confidence transition, i.e. the test/metrology layer is where CPO *volume* gets gated (ties the [[COHR]]/[[LITE]]/[[GLW]] optical thesis to a manufacturing constraint, not just demand). **(2) China memory capex:** Yole — **CXMT launched ~US$5–6B equipment tenders for a 50–60k wspm capacity add; YMTC began a Phase-2 process-equipment tender (May 14)** — synchronized memory-giant expansion driving etch/deposition/CMP orders; the supply-build behind the [[MU|CXMT-climbs-the-stack]] thread (commodity tier, 2–3yr horizon, "still far from flooding"). **(3) Goertek AR optical plant** (Shanghai Lingang): China's first large-scale **12-inch transparent-substrate** fab for AR micro-nano optics in commercial production (~$457M, nanoimprint litho, AR waveguide/electrochromic/microlens) — AR/smart-glasses optics supply-build, photonics-adjacent. **(4) Research:** "HBM Is Not All You Need" (arXiv) — disaggregated serving puts cheap GDDR accelerators on the HBM-idle *prefill* phase, a marginal HBM-demand-relief vector (sibling to [[2026-06-23-semiwiki-daily|AMD's MEXT]] memory-tiering). **Which means:** the binding constraint keeps widening past GPUs — photonics test, China equipment demand, and substrate/optics layers — while two independent efficiency vectors (MEXT, MemHA) nibble at HBM intensity at the margin. No stance changes.
