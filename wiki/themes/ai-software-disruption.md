...
- **2026-06-25 — The labor side of the disruption, from an operator: AI cuts the *measurers*, not the builders/sellers.** Per [[2026-06-25-cloudflare-prince-agentic-web]] (Cloudflare CEO Matthew Prince), Cloudflare cut **>20% of staff** — "not because the business was struggling" — because "we need fewer middle managers, fewer **measurers**." Span of control going 6:1 → 12:1 (Meta targeting 50:1); IR earnings-doc prep went 2 weeks → 3 minutes; internal audit moving from sampling 6-10 of 105 risk areas to continuously auditing all 105. His framing: AI replaces the *measuring/coordination* layer, while builders and sellers get *more* valuable ("I'll hire as many developers/salespeople as I can"). "Next 6-12 months almost every company goes through this." **Which means:** a concrete, operator-stated version of the survival fault line — corroborates Oracle's −21k (capex-driven, not AI-replacement, but same direction) and extends this page's question from *which software gets eaten* to *which roles inside every company get eaten* (the answer rhymes: coordination/measurement layers, not creation/distribution). Bias: CEO justifying his own layoff; but the mechanism is specific and falsifiable.

- **2026-06-26 — Two app-layer reads from an OpenAI scientist: per-task cost deflation, and routing-middleware moat doubt.** Per [[2026-06-26-nopriors-noam-brown-test-time-compute]] (Noam Brown — *bias-flagged: OpenAI insider*): (1) the cost of any given capability falls **10–100× per model release cycle** (cycles now ~2–3 months) — deflationary for app-layer unit economics, but TAM-expanding as new tasks become affordable (the Jevons/"growth re-rate not replacement" reconciliation below, from the model side); (2) **routing-layer skepticism** — companies whose value-prop is the *router/choice layer* ("which model, how much inference per subtask") may not beat just letting one frontier model think longer once you control for test-time compute; benchmark gains from routing/consensus often don't survive real-world use. **Which means:** a caution on the durability of "model-router middleware" as an application moat — the routing Brown expects to be absorbed behind the lab API and inside the model itself, not captured by a thin orchestration layer. Pairs with the routing-economics note in [[inference-economics]].
...
- **2026-06-26 — the reversal trigger appears to have fired (durability test in progress).** **IGV +~4%, MSFT +5%** as the memory/semi trade stalled (MU blew off +15.7% June 25; KOSPI rolling over) and OpenAI's reported **IPO-delay-to-2027** cooled the AI-capex/AGI trade — capital rotated out of crowded AI-hardware into beaten-down software, exactly the pre-registered trigger ([Motley Fool](https://www.fool.com/investing/2026/06/26/why-microsoft-stock-rose-today/)). **But discount it:** the move is *below* IGV's 50-DMA (IGV closed $84.76; it lost the 50-DMA on June 17, so this is a bounce *inside* a downtrend), the **10Y was flat (~4.39%) so it was NOT a rate/duration rally**, and there was **no fundamental software catalyst** (no print showing consumption > seat-erosion) — so it reads as much like a bear-market rally / oversold-snap + quarter-end rebalancing as a regime change. Three falsifiable durability tests logged in `newsletter/predictions.json` (2026-06-26): does it stall below ~$88, does it fade post-quarter-end, and is it re-discrimination vs. a short-squeeze. **Verdict pending early-July follow-through.**
...
- **The app-layer winners moving *down* the stack into their own models** — datapoint (2026-06-24): Cursor's Sasha Rush detailing how **Composer 2.5** was trained with novel RL methods ([[2026-06-24-dwarkesh-sasha-rush-on-policy-distillation]]) confirms the strongest coding-agent player is building a *vertical in-house frontier model*, not wrapping someone else's. **Which means** the durable margin at the app layer may accrue to those who own both the workflow *and* the model — a moat the pure-seat incumbents (CRM) don't have and the pure-wrappers can't defend.
  - **2026-06-29 — the verticalization is going *ecosystem-wide and simultaneous* ("pretrainitus"), and a Chinese-model-ban risk is an accelerant (unattributed practitioner observation; reasoning, not a sourced datapoint).** The sharper version of the Cursor point: *every* revenue-generating AI app (Cursor, Perplexity, the coding/agent leaders) is rushing to own its model layer at once. **Two precisions:** (1) it's mostly **post-training / distillation / RL on open-weight bases**, *not* full $100M+ pre-training from scratch — "owning a model" via verticalization, not literal pretraining; (2) the **main drivers are pricing-power-inversion** (closed labs raising prices — Opus +40%, GPT-5.5 2× — see [[inference-economics]]) **+ defensibility** (a wrapper has no moat), with the **Chinese-model-ban fear an *accelerant*, not the cause.** **The irony worth flagging:** these in-house models are frequently built *on the very Chinese open-weight bases* (Qwen/DeepSeek/GLM) they fear losing — so a US ban is **double-edged** (it disrupts their own-model strategy too, forcing a fallback to laggier Western open-weight). **The investable read-through:** distributed model-ownership across the whole app layer = a *new, dispersed source of training/post-training compute demand* (bullish picks-and-shovels — [[NVDA]]/[[CRWV]]/[[NBIS]] + the post-training/RL-environment toolchain) **and** continued erosion of frontier-lab API pricing power (the [[ai-bubble-debate|value-migration]] thesis). If a Chinese-model ban actually fires → bullish US frontier labs + Western open-weight + the own-model push; bearish the cheap-Chinese-inference arbitrage.
