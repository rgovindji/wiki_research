...
- **Watch-item critical material: tungsten / WF₆ (NEW 2026-06-29).** Per [[2026-06-29-semianalysis-tungsten-wf6-bottleneck]] (SemiAnalysis X thread — *moderate bias, sells research, inclined to surface "the next bottleneck"*): tungsten fills the deep high-aspect-ratio **vias** in fab CVD (demand rises with 3D-NAND stacking + more metal layers), and supply is tightening — **China dominates the tungsten raw chain**, Japan's WF₆ makers (SK Materials, Shin-Etsu) are squeezed on falling raw imports, and **Korean WF₆ import prices are +151% YTD.** **Which means:** another China-controlled critical input swept into the export-control fight (see [[us-china-relations]]) and another AI-hardware cost-push — *but* it's the textbook **"esoteric bottleneck"**: real yet hard to monetize (tiny cost-share; suppliers buried in conglomerates). **The framing that reconciles it with Baker's "it's DRAM, not the esoteric stuff" (above):** DRAM is where *pricing power/capital* accrues (the biggest binding constraint); WF₆/helium are *tail-risk/fragility* inputs (small cost, but a China supply-shock could gate fab throughput). Log as a **risk flag, not a position**; the only semi-clean exposure is ex-China tungsten supply (speculative miners, e.g. Almonty-type — *verify*), not the diversified gas names ([[ENTG]]/LIN/[[APD]]).
...
- **2026-06-26 — A demand-response release valve on the power bottleneck: ~100 GW of US "flexible power."** Per [[2026-06-26-semianalysis-gpu-power-curve-pebble]] (Pebble @ GTC — *founder talks-his-book*): if data centers can demonstrate they'll **curtail power at peak periods** (go grid-responsive instead of static base-load), there is **~100 GW of "flexible power" available in the US** to tap "for certain trade-offs." **Which means:** part of the 2026-28 power chokepoint is addressable *without new generation* — software-side demand-response can stretch existing grid headroom, a partial counter to the hard "I have chips I can't plug in" wall. Watch for hyperscaler demand-response / curtailable-PPA deals as confirmation. The same source confirms the *other* edge of the power story: the GPU power→token curve is **non-linear/saturating** and inference is memory-bound (decode = HBM weight reads), so effective tokens-per-watt sit below nameplate — see [[inference-economics]] / [[ai-bubble-debate]].

- **2026-06-28 — "Goodput," not $/GPU-hour, is the neocloud quality moat — and effective compute < nameplate (SemiAnalysis).** Per [[2026-06-28-semianalysis-true-cost-gpu-cluster]] (*bias MODERATE: funnels to SemiAnalysis's ClusterMAX ranking*): two clusters with identical GPUs/generation/headline price can differ **10–50% in real TCO** once you count storage, networking, control plane, support, **setup** (days–weeks of billed tuning), **debugging**, and above all **goodput** (useful work after failures/restarts/downtime). Reliability *worsens* with scale — at >1,000 GPUs failures are normal operation (one rank hang stalls a whole comm group), so the real buyer question is "how much goodput should I expect?" not "$/GPU-hr." The emerging training fault-tolerance stack: **TorchFT** (PyTorch), **AWS SageMaker HyperPod checkpointless training**, **Clockwork TorchPass**. **Which means:** (1) a second-order reason *effective* compute supply lags installed base — a goodput "tax" on top of the ~60–70% MFU waste and the saturating power→token curve above; and (2) the durable differentiation for premium neoclouds ([[CRWV]], [[NBIS]], [[CRUSOE]], [[WYFI]]) is reliability/goodput, which the headline rental price hides — supports paying up for ClusterMAX-tier operators over the cheapest sticker. See [[inference-economics]], [[ai-capex-cycle]].

- **2026-06-29 — A heavyweight says stop hunting esoteric bottlenecks — it's DRAM, full stop (Gavin Baker, All-In #278).** Per [[2026-06-29-allin-278-china-coding-micron]] (*bias: long MU; AI-maximalist*): Baker derides the "**bottleneck bros**" who use Claude to find esoteric Japanese suppliers and argues **the one bottleneck that matters is DRAM** — "not lasers, not capacitors, not power-supply semiconductors, not NAND, not HDDs" — because memory capacity + bandwidth are foundational to every model (Elon is pointing Terrafab at memory for this reason). He sizes it: **DRAM ≈ 30-40% of all hyperscaler capex next year.** **Which means** a heavyweight prioritization that *tensions this page's own multi-bottleneck / esoteric-supplier framing* (the SOITEC / AXTI / ABF-substrate-style picks) — a useful corrective to weight the *biggest* chokepoint over the cleverest one, though discount it as a simplification from someone long the memory names. Two supply nuances he adds: **CXMT (China DRAM IPO) floods CONSUMER-grade DRAM, not AI-grade** (HBM / SOCAMM / LPDDR — only 3 firms can make it), containing the China-flood bear to the commodity tier; and **inference disaggregation** (decode-optimized Grok/Cerebras chips in front of old GPUs) extends usable GPU life to **7-12 years**. Reinforces [[MU]] and the memory rows. See [[inference-economics]].
