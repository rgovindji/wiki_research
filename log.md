...
## [2026-06-30] note | YouTube sweep — thin window, no new transcribe (quality-over-volume); Zitron token-billing bear logged un-transcribed
- Sweep mechanics: RSS roster (yt_rss.sh --roster 5) + 4 discovery passes (Tier-1 majors / Tier-5 bears / theme / conference-enumeration). The orchestrating discovery sub-agent stalled in a nested-delegation wait loop (same watchdog failure noted 6/29) — re-ran all four passes inline + via four parallel lane-agents.
- RESULT: genuinely THIN window (6/25–6/30). NO new YouTube video cleared the relevance filter with a verifiable, transcribable 11-char ID:
  - **RSS roster** = nothing new. All in-window hits already ingested (Mark Chen fpAthTtha8c 6/28, Noam Brown AZrU6y3pUcU 6/26, SemiAnalysis true-cost-GPU whvik_QgN30 6/28, Pebble OpL-Q0YAKJU 6/26) or dropped (NanoClaw hLUGXO5DSpo, Unitree uzl5tUHIFXA). The one new ID — No Priors "AI benchmark scores don't tell you what you think" (ta3Y6AdfGxI) — TRANSCRIBED (146w) then DROPPED: it's a promo SHORT excerpted verbatim from the already-ingested Noam Brown episode.
  - **Tier-1 majors** = clean negative. Closest = Dario Amodei Bloomberg "The Circuit" (x2VHFgyawPE, 6/17) + Jensen CNBC AI-policy (t9RPhPnfNLs, 6/17) — both out-of-window + already-covered themes. Jensen's only recent keynote is Computex/GTC-Taipei (early June, ingested-era). C.C. Wei 6/4 + Demis 6/23/6/29 talent-war items are TEXT articles, not videos.
  - **Conference (AIWF 2026, June 29–July 2 SF)** = talks NOT uploaded yet (post AFTER the event). Only the multi-hour Day-2 livestream (4sX_He5c4sI) exists — NEW but not specific (fails filter). **ACTION: re-run conference enumeration ~July 6–10 on @aiDotEngineer** for the lab-keynote / inference-economics-track talks.
  - **Theme** = a Micron-earnings reaction cluster (R1jsbfouRQY, sNaPyhM8QGM, qHjyytqxDkw, ~6/24–25) — recap/reaction format, and MU's print is ALREADY ingested (sources/2026-06-24-MU-Q3-FY26-earnings.md). Skipped per filter (no analyst-grade teardown confirmed). All keyword hits (inference econ / memory shortage / SMR / liquid cooling) date Feb–Apr 2026 → fail NEW.
  - **Bears** = only in-window high-signal item is Ed Zitron "Tokenpocalypse Now" (Better Offline, 6/26) — but it's an AUDIO podcast (iHeart/Podbean/Apple) with NO confirmable YouTube video ID, and Zitron is already the most-saturated bear in the ledger. Not transcribed (won't invent an ID).
- Wiki touched: ai-bubble-debate (last_updated 6/30; added a 6/30 Zitron token-billing/ROI bear bullet — externally cited to the iHeart podcast page, explicitly flagged podcast/un-transcribed + talks-his-book; sources held at 51 since no source-summary file was created). No source summaries written. No index.md change (no new source ingest). No stance/conviction changes anywhere.
- Falsifiable call captured (Zitron, flagged for predictions.json — NOT auto-logged): "the shift to usage/token-based billing exposes that enterprises can't measure AI ROI → bubble bursts" — gradeable against (a) net-revenue-retention / churn at the model labs through 2026–27 and (b) whether per-token monetization (monetization-per-GW) keeps rising (the bull counter) or rolls over. Directly testable against the [[inference-economics]] monetization-per-GW series.

## [2026-07-01] ingest | YouTube sweep — Dylan Patel (Sequoia HW-SW co-design) + Aswath Damodaran (AI valuations / SpaceX)
- Sweep mechanics: RSS roster (yt_rss.sh --roster 5) + 3 parallel discovery agents (Tier-1/2 majors · Tier-5 bears + theme · conference/Tier-3-6). RSS = nothing new (NoPriors -kMLjDGZ33o + ta3Y6AdfGxI = 165w/146w promo Shorts off the already-ingested Noam Brown ep; a16z "creative" off-mandate; Dwarkesh = Ada Palmer history series off-topic; SemiAnalysis whvik_QgN30 already ingested 6/28). Tier-1/2 = clean negative (Amon Fox Business the only in-window hit — broadcast segment, fails bar). AIWF 2026 (June 29–Jul 2) talks NOT uploaded yet → re-sweep @aiDotEngineer ~July 3–5. 2 transcribed, both clear keepers.
- (1) **Dylan Patel × Sequoia (f6D_aiy8qyU, 15,650w).** sources/2026-07-01-sequoia-dylan-patel-hw-sw-codesign.md. Bias-flagged: Sequoia hosts (Sean/Sonia Huang) large SpaceX investors + long Crusoe / "Navian" co-design startup; Patel sells InferenceX + loves SpaceX. Signal: **co-design = the real 100x** (isolated HW/systems/model gains ~2x each ≈8x; co-optimize all three = 100x; equiv-quality cost −60x/yr, intelligence-per-watt −40x/yr, Hopper→Blackwell ~30x on DeepSeek). **Hard per-GW rental table** (cleanest neocloud dataset yet): Trainium <$10B/GW, GPU $12–13B/GW, SpaceX→Google ~$25B/GW (running-now premium); colo power-only $60→$120–160/kW-mo. **Anthropic** Q2 profitable ex-SBC (maybe incl-SBC by Q3), >80% Opus-token GM → can pay above-market for every GPU and still grow NOI (strongest rebuttal yet to Chanos/Damodaran "neocloud = 4–8% ROIC finance co" — but describes the *lab*, not the *landlord*). **Google runs 3 distinct TPU architectures** (Broadcom/MediaTek/undisclosed); everyone gets an ASIC program but keeps NVDA as general-purpose hedge (local-minima risk). CUDA moat "partially disentangled" (models write kernels; real lock = open models co-designed for GPUs). Jensen funds neoclouds + China labs for a multipolar world. **Memory:** no cell breakthrough in decades → memory-on-logic next; 1 W/mm² power-density wall (Rubin ~2kW, Rubin Ultra ~4kW). **Neoclouds exist** because AI GPU-rental mechanics stripped hyperscaler CPU-cloud moats; CoreWeave tests better on perf/reliability; Crusoe flagged a customer asking to HALT a buildout (first demand crack). **Crunch:** 20 GW 2026 → >30 GW 2027; falsifier = "model progress stops."
- (2) **Aswath Damodaran (6XOjrkb47AA, 4,854w, Indian-media interview).** sources/2026-07-01-damodaran-ai-valuations-spacex.md. Low-bias (valuation professor, no book; ~40% off-mandate India/FII content). Signal: **SpaceX $1.3T intrinsic vs >$1.8T price = overpriced** ("great company that's not [currently] a great investment"; Musk net-neutral on price). **Architecture-not-products** — durable winner won't be infra (Amazon, not Cisco); money "next" is products/services, maybe not-yet-public. **AI is real+huge but that ≠ profitable** ("big market where competition is so intense nobody makes money"). **Only systemic worry = debt-funded small/mid-cap architecture** (NOT Mag-7/NVDA) — "they take some of us down with them" (independent low-bias corroboration of Chanos frame → ai-infrastructure-debt). **AI ≠ carrying the market** (strip AI names, still richer than pre-AI; held while NVDA/Mag-7 sold off = real breadth). Bear-inside-bull: ~$26T prospectus TAM ⇒ ~1 in 2 white-collar jobs lost ("you don't *want* the best-case AI scenario").
- Wiki touched: inference-economics (21→22), bottleneck-roadmap (24→25), ai-capex-cycle (27→28), ai-bubble-debate (51→53, both sources), ai-infrastructure-debt (4→5), valuation-environment (1→2), NVDA (14→15), CRWV (7→8), MU (28→29), CRUSOE (8→9), SPCX (2→3, both sources), AVGO (3→4), index (Recent ingests + Sources), log. No stance/conviction changes anywhere.
- Top portfolio reads: (a) monetization-per-GW / >80% Opus GM = strongest bull rebuttal yet to the neocloud-ROIC short, but it's the LAB's P&L not the landlord's — the fork stays open; (b) Damodaran + Patel bracket the bubble debate cleanly (Damodaran: the *leverage* is the fault line; Patel: *model-progress margin* keeps leverage serviceable) → the real disagreement is how perishable the model-progress bid is; (c) NEW near-term watch: Crusoe customer asked to HALT a buildout — first visible demand crack; track the levered mid-cap builder cohort.
- Candidate falsifiers for predictions.json (flagged, NOT auto-logged): (1) Patel "20 GW deployed 2026, >30 GW 2027" — checkable vs SemiAnalysis/industry DC-commissioning trackers. (2) Patel "Anthropic profitable ex-SBC in Q2, possibly incl-SBC by Q3; >80% Opus-token GM" — testable vs any Anthropic financial disclosure/leaks. (3) Patel per-GW rental spread (Trainium <$10B / GPU $12–13B / SpaceX $25B) — directional, checkable vs future neocloud contract disclosures. (4) Damodaran "SpaceX intrinsic $1.3T vs >$1.8T price" — gradeable vs where SPCX trades over 6–18mo. (5) Damodaran "~1 in 2 white-collar jobs must go to justify the $26T prospectus TAM" — long-dated macro, not near-term gradeable.
