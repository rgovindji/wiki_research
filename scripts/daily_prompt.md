...
1. **Discover** — parallel WebSearch sweeps:
   ...
   - **Multi-source feeds (daily, mandatory): run `python3 scripts/feed_scan.py`** — returns NEW items since last run, **grouped by persona** (`builder` = HuggingFace trending models + HN AI-infra: what's shipping & what's hard; `researcher` = arXiv-hardware + HF daily papers: the frontier; `investor` = Damodaran + HN-markets: valuation/forecasting; `operator` = SemiAnalysis: supply-chain reality). Read every persona block. For any item worth more than its snippet, `WebFetch` the URL. Same signal-vs-noise bar + diamond-in-the-rough rule as SemiWiki. Capture builder demand/pain signals (what compute/memory/tooling they need) and investor valuation framings even when they don't move a stance — they're the leading indicators. (`builder` also pulls **r/LocalLLaMA**; `investor` also pulls **r/stocks + r/wallstreetbets + r/investing** — Reddit's .json is dead but its .rss flows via the Webshare residential SOCKS5 proxy + curl.)
...
python3 scripts/factor_tape.py
```

`factor_tape.py` gives the cross-asset drivers (10Y, oil, dollar, VIX, gold) with today's move + 5-day trend — the inputs the **Reaction Desk** reasons over (part 4). Record the day's dominant driver (or "no clean driver") + the cross-asset levels in a `factor_attribution` block in the market_state snapshot.
...
4. **Log new calls**: every falsifiable take in today's letter gets an entry — claim, type, confidence, horizon, explicit falsifier. **Max 3 new predictions per issue.** No falsifier, no call: rewrite the take until it has one. This includes the **Reaction Desk's conditional warning** (type `reaction`): the reader sees the plain version ("hawkish Fed → mega-caps down"); the ledger gets the testable one with an explicit trigger/level and the resolving event's date as the horizon, so the desk's hit-rate is scored.
...
6. **Reaction Desk** — run the method in `.claude/agents/reaction-desk.md`: forward-looking first (next ~2 weeks of events → simple directional decision-branches for mega-caps/sectors), falling back to today's dominant driver, falling back to "mixed, no clean driver — don't over-read." **Plain and directional — arrows and sector names, no mechanism lectures.** ≤1 compact block (the agent's output format). It can be invoked as a standalone subagent too, but inline is fine for the letter.
7. **Optional, max once or twice a week: a high-risk idea** — clearly framed as speculative: the setup, the entry logic, the explicit invalidation ("wrong below X"), the payoff shape, and sizing language ("money you can lose entirely"). Always logged to the ledger so it gets scored like everything else. Never added to the model portfolio — holdings changes are the user's call. Run any small-cap through the playbook's rug-signature check first.
8. **Things to watch** — a small markdown table of tomorrow/this week's catalysts (When | What | Why it matters). Only rows that matter.
9. **Optional: One Concept Worth Knowing** — when the day's news used a mechanic worth unpacking (include it most days; skip when forced).
10. **The bottom line** — a short closing section that lands the day's thesis. Sign off "— *After Hours*".
